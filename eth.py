...
import numpy as np
import schedule
json_file_path = 'eth_predictions.json'
...
def train_eth_model(X, y):
    model = make_pipeline(StandardScaler(), Ridge(alpha=0.1))
    model.fit(X, y)
    return model

def train_eth_random_forest_model(X, y):
    model = RandomForestRegressor(n_estimators=100, random_state=42)
    model.fit(X, y)
    return model

def train_eth_gradient_boosting_model(X, y):
    model = GradientBoostingRegressor(n_estimators=100, learning_rate=0.1, max_depth=3, random_state=42)
    model.fit(X, y)
    return model

def make_eth_predictions(models):
    timestamps, _ = fetch_eth_historical_data()
...
def main():
    models = {}
    timestamps, prices = fetch_eth_historical_data()

    if timestamps and len(timestamps) > 0:
        X = generate_features(np.array(timestamps).reshape(-1, 1))
        y = np.array(prices)

        models['Ridge'] = train_eth_model(X, y)
        models['RandomForest'] = train_eth_random_forest_model(X, y)
        models['GradientBoosting'] = train_eth_gradient_boosting_model(X, y)

    # Schedule predictions every hour
    schedule.every().hour.at(":00").do(make_eth_predictions, models=models)
