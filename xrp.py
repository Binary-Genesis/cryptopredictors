...
import numpy as np
import schedule
json_file_path = 'xrp.json'
base_url = 'https://api.coingecko.com/api/v3'
api_key = 'API_KEY'

def get_xrp_price():
    url = f'{base_url}/simple/price?ids=ripple&vs_currencies=usd'
    headers = {
...
def train_xrp_model(X, y):
    model = make_pipeline(StandardScaler(), Ridge(alpha=0.1))
    model.fit(X, y)
    return model

def train_xrp_random_forest_model(X, y):
    model = RandomForestRegressor(n_estimators=100, random_state=42)
    model.fit(X, y)
    return model

def train_xrp_gradient_boosting_model(X, y):
...
def make_xrp_predictions(models):
    timestamps, _ = fetch_xrp_historical_data()
...
def main():
    models = {}
    timestamps, prices = fetch_xrp_historical_data()

    if timestamps and len(timestamps) > 0:
        X = generate_features(np.array(timestamps).reshape(-1, 1))
        y = np.array(prices)

        models['Ridge'] = train_xrp_model(X, y)
        models['RandomForest'] = train_xrp_random_forest_model(X, y)
        models['GradientBoosting'] = train_xrp_gradient_boosting_model(X, y)

    # Schedule predictions every hour
    schedule.every().hour.at(":00").do(make_xrp_predictions, models=models)
