...
import numpy as np
import schedule
json_file_path = 'matic.json'
base_url = 'https://api.coingecko.com/api/v3'
api_key = 'API_KEY'

def get_matic_price():
    url = f'{base_url}/simple/price?ids=polygon&vs_currencies=usd'
    headers = {
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            return data.get('polygon', {}).get('usd')
        else:
            return None
...
def train_matic_model(X, y):
    model = make_pipeline(StandardScaler(), Ridge(alpha=0.1))
    model.fit(X, y)
    return model

def train_matic_random_forest_model(X, y):
    model = RandomForestRegressor(n_estimators=100, random_state=42)
    model.fit(X, y)
    return model

def train_matic_gradient_boosting_model(X, y):
    model = GradientBoostingRegressor(n_estimators=100, learning_rate=0.1, max_depth=3, random_state=42)
    model.fit(X, y)
    return model

def make_matic_predictions(models):
    timestamps, _ = fetch_matic_historical_data()
...
def main():
    models = {}
    timestamps, prices = fetch_matic_historical_data()

    if timestamps and len(timestamps) > 0:
        X = generate_features(np.array(timestamps).reshape(-1, 1))
        y = np.array(prices)

        models['Ridge'] = train_matic_model(X, y)
        models['RandomForest'] = train_matic_random_forest_model(X, y)
        models['GradientBoosting'] = train_matic_gradient_boosting_model(X, y)

    # Schedule predictions every hour
    schedule.every().hour.at(":00").do(make_matic_predictions, models=models)
